...
@ray.remote
def parallel_get_pixel_class(
        points, ds_prev, labels_from_last_iteration, regrow_kernel, xmax, ymax, start_index, BORDER):
    results = {}  # A map of location (i,j) and it's corresponding label
    for (i, j) in points:
        neighbors = ds_prev[max(i - regrow_kernel, 0): min(i + regrow_kernel + 1, xmax),
                            max(j - regrow_kernel, 0): min(j + regrow_kernel + 1, ymax)]
        old_neighbor_label = labels_from_last_iteration[max(
            i-regrow_kernel, 0):min(i+regrow_kernel+1, xmax), max(j-regrow_kernel, 0):min(j+regrow_kernel+1, ymax)]
...
def parallel_regrow_to_original_size(
        clipped_labels_with_background, labels_from_last_iteration, start_index, cpu_count, BORDER=1):
    ...
    while True:
        ...
        splits = np.arange(1, len(points), int(len(points)/cpu_count))
        indx_s = [(splits[i-1], splits[i]) for i in range(1, len(splits))]

        # Do the main processing in parallel
        result_ids = [
            parallel_get_pixel_class.remote(
                points[i: j],
                ds_prev, labels_from_last_iteration, regrow_kernel, xmax, ymax, start_index, BORDER) for(i, j) in
            indx_s]
...
def iteratively_erode_and_separate_objects(
        img_grey, min_size, clip_distance_list, min_instance_size, cpu_count, BORDER=1, BACKGROUND=0):
    """Separate into smaller trees

    Args:
        img_grey (2D np array): Segmentation mask on which the spliting is performed
        min_size (int, optional): Minimum size of the instance to be considered for spliting. Defaults to 500.
    ...
    for cdt in clip_distance_list:
        ...
        if cpu_count <= 1:
            ...
            regrown_labels = regrow_to_original_size(
                clipped_labels_with_background, instance_to_split_further, first_free_index, BORDER=1)
        else:
            logging.info(f"Regrowing labels using {cpu_count} CPU cores.")
            regrown_labels = parallel_regrow_to_original_size(
                clipped_labels_with_background, instance_to_split_further, first_free_index, cpu_count, BORDER=1)
        ...
        for u in unq_regrwn[1:]:
            ...
            if np.count_nonzero(ins_u) > min_size:
...
def separate_images_in_dir(
    ...
    for file in files:
        ...
        if not os.path.isfile(out_split_instances) or force_overwrite:
            start = time.time()
            sp = iteratively_erode_and_separate_objects(
                img, min_size, clip_distance_list, min_eroded_instance_size, cpu_count)
...
if __name__ == '__main__':
    args = utils.get_args('erosion_separation')
